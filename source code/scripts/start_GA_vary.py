...
if __name__ == '__main__':
    ...
    # Starts the main training loop: the one about the episodes to do
        ...
        record_gait.fill(0)
        record_mu[0,[0,3,6]]            = np.random.uniform(1,4)
        record_mu[0,[1,2,4,5,7]]        = np.random.uniform(0,1)

        if not os.path.exists(outdir+str(z+1)):
            os.makedirs(outdir+str(z+1))
        if not os.path.exists(outdir+str(z+1)+'/temp'):
            os.makedirs(outdir+str(z+1)+'/temp')

        while x < n_evaluation:
            if x==0:
                pop_params = GA.first(record_mu[0,:])
                pd.DataFrame(pop_params).to_csv(outdir+'/'+str(z+1)+'/rollout_param.csv',index=False,header=False,float_format='%.6e',mode='w')
            else:
